...
const SectionHeading: React.FC<{ children: React.ReactNode; id?: string }> = ({ children, id }) => (
  <h2 id={id} className="text-3xl md:text-4xl font-extrabold text-blue-950 mb-8 flex items-center gap-3">
...
const StatCard: React.FC<{ icon: React.ReactNode; label: string; value: string; description: string }> = ({ icon, label, value, description }) => (
  <div className="group p-8 rounded-3xl bg-gray-50 border border-gray-100 hover:bg-white hover:shadow-xl hover:border-transparent transition-all duration-300">
    ...
    <span className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">{label}</span>
...
const App: React.FC = () => {
  ...
  const generateExcuse = () => {
    if (!data) return;
    const newExcuse = data.excuses[Math.floor(Math.random() * data.excuses.length)];
    setExcuse(newExcuse);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="relative bg-white pt-20 pb-16 px-6 overflow-hidden border-b border-gray-100">
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          <p className="text-xl md:text-2xl text-gray-600 font-medium max-w-2xl mx-auto mb-12">
            ...
            <span className="text-sm text-gray-400 mt-2 block font-normal">Updated through the {data.updatedThroughSeason} season</span>
          </p>
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 mb-12">
            <div className="flex items-center gap-8 bg-white p-8 rounded-3xl shadow-xl border border-gray-100 w-full md:w-auto">
              <div className="text-center px-4">
                <span className="block text-xs font-bold text-green-700 uppercase tracking-widest mb-1">Packers</span>
                <span className="text-6xl font-black text-blue-950"><CountUp end={data.allTime.packersWins} /></span>
              </div>
              <div className="h-16 w-[1px] bg-gray-200"></div>
              <div className="text-center px-4">
                <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Ties</span>
                <span className="text-3xl font-bold text-gray-300"><CountUp end={data.allTime.ties} /></span>
              </div>
              <div className="h-16 w-[1px] bg-gray-200"></div>
              <div className="text-center px-4">
                <span className="block text-xs font-bold text-blue-950 uppercase tracking-widest mb-1">Bears</span>
                <span className="text-6xl font-black text-blue-950"><CountUp end={data.allTime.bearsWins} /></span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto">
            <div className="bg-gray-50 p-4 rounded-xl flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-500 uppercase">Win % (GB)</span>
              <span className="text-lg font-bold text-green-800">{(data.allTime.packersWinPct * 100).toFixed(1)}%</span>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-500 uppercase">Last Matchup</span>
              <span className="text-lg font-bold text-blue-950">{data.allTime.lastMatchup.winner}</span>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <StatCard 
              ...
              label="Games Played"
              value={(data.allTime.packersWins + data.allTime.bearsWins + data.allTime.ties).toString()}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="space-y-4">
            {data.eras.map((era, idx) => (
              <div key={idx} className="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md transition-shadow">
                <div className="md:w-1/3">
                  <h3 className="text-2xl font-bold text-blue-950 mb-2">{era.name}</h3>
                ...
                <div className="flex items-center justify-center gap-12 bg-gray-50 px-8 py-4 rounded-2xl md:w-auto">
                  <div className="text-center">
                    <span className="block text-[10px] font-bold text-green-700 uppercase mb-1">Packers</span>
                    <span className="text-3xl font-black text-blue-950">{era.packers}</span>
                  </div>
                  <div className="text-gray-300 font-bold text-xl">vs</div>
                  <div className="text-center">
                    <span className="block text-[10px] font-bold text-blue-950 uppercase mb-1">Bears</span>
                    <span className="text-3xl font-black text-blue-950">{era.bears}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="excuses" className="py-24 px-6 bg-white overflow-hidden relative">
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="text-center mb-12">
            <h2 className="text-4xl md:text-5xl font-black text-blue-950 mb-4 tracking-tight">The Bears Excuse Generator</h2>
            <p className="text-gray-600 font-medium">Lost again? Don't worry, we've got you covered with a ready-made reason.</p>
          </div>
      ...
      <section id="gear" className="py-20 px-6 bg-gray-50">
        <div className="max-w-4xl mx-auto bg-white rounded-3xl p-10 md:p-16 text-center border border-gray-100 shadow-sm">
          ...
          <p className="text-gray-500 mb-10">Rep the green and gold (or the navy and orange) for the next showdown. Don't show up empty-handed.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <a href={data.ctaLinks.ticketsUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 px-8 py-5 bg-blue-950 text-white font-bold rounded-2xl hover:translate-y-[-2px] transition-all">
              <Ticket className="w-5 h-5" />
              Find Tickets
            </a>
            <a href={data.ctaLinks.gearUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 px-8 py-5 border-2 border-blue-950 text-blue-950 font-bold rounded-2xl hover:bg-blue-50 transition-all">
              <ShoppingBag className="w-5 h-5" />
              Shop Rivalry Gear
            </a>
          </div>
          <p className="mt-8 text-[10px] text-gray-400 uppercase tracking-widest">Links may be affiliate links in the future.</p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 border-b border-white/10 pb-12 mb-12">
            <div className="text-center md:text-left">
              <span className="text-2xl font-black tracking-tight block mb-2">PackersVsBears.com</span>
              <p className="text-white/50 text-sm max-w-sm">
                Providing essential rivalry stats and lighthearted satire for the NFL's most legendary matchup.
              </p>
            </div>
            <nav className="flex items-center gap-6">
              <a href="#stats" className="text-sm font-semibold hover:text-orange-500 transition-colors">Stats</a>
              <a href="#eras" className="text-sm font-semibold hover:text-orange-500 transition-colors">Eras</a>
              <a href="#excuses" className="text-sm font-semibold hover:text-orange-500 transition-colors">Excuses</a>
            </nav>
          </div>
          <div className="text-center text-[10px] text-white/30 uppercase tracking-[0.2em] leading-relaxed">
            ...
            <p>
              Fan-made satire + stats. Not affiliated with the NFL or any team. <br/>
              Team names and statistics used for informational/reporting purposes only.
            </p>
          </div>
        </div>
      </footer>
